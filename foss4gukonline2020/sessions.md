_This is the provisional listing of sessions (in alpha order by lead presenter) for FOSS4G UK Online 2020 - the full programme will be released soon. In the meantime the listings below are subject to change over the coming weeks._
...
# Optimized publishing of map and dataservices with GeoServer, Gestyler, Geowebcache and MapProxy (presentation)
...
OpenAQ is an environmental tech non-profit that has created the world’s first open, real-time and historical air quality platform, aggregating governmental and research-grade data -- entirely open-source. The platform has over 500 million data points from 93 countries and serves 33 million API requests per month. The data have been used for a wide variety of applications, from air quality forecasts produced by NASA scientists to platforms communicating air quality in India to data-driven media reports by the general public. By providing this foundational data infrastructure, OpenAQ is able to convene people and organizations from across the globe to further raise awareness and develop innovative solutions to combat air pollution. The talk will give a technical overview of the platform, highlight the impact through user stories, and feature new tools we have developed with the community to enhance the platform and use the data to fight for change. The OpenAQ platform itself is a geospatial tool since all of the data is geo-tagged. I'll also be talking about a new averaging tool we've developed on top of the platform that allows someone to get pollutant averages across various spatial and temporal resolutions via the API. And because it's open source, the averaging tool can be used for other point data.
